...
void readCmdLineVar(Variable* var, int argc, char const* argv[]) {
    ...
    bool nocite = true, showFileInfo = false, cvt2lmpFlag = false;
    for (int iarg = 1; iarg < argc; iarg++) {
        ...
        } else if (!strcmp(argv[iarg], "--showInfo")) {
            showFileInfo = true;
        } else if (!strcmp(argv[iarg], "--cvt2lmp")) {
            cvt2lmpFlag = true;
...
#if defined(Harmonic)
            safeFprintf(logFile, "\tInteraction between particles: Harmonic\n");
#elif defined(Hertzian)
            safeFprintf(logFile, "\tInteraction between particles: Hertzian\n");
#else
...
#error "No Interaction is defined!"
    ...
    if (cvt2lmpFlag) {
        ...
        writeLammpsTopo(box, particle, update, var);
    }
    if (showFileInfo) {
        Box* box = (Box*)calloc(1, sizeof(Box));
        Particle* particle = (Particle*)calloc(1, sizeof(Particle));
        ...
        showInfo(box, particle, update);
        showStructInfo(box, particle, update);
    }

    if (cvt2lmpFlag || showFileInfo) {
        exit(EXIT_SUCCESS);
    }
}
...
void readConf_data(Box* box, Particle* particle, Update* update, Variable* var) {
    ...
    char str[4096];
    fgets(str, 4096, fp);
    if (strstr(str, "binary")) {
        int binType = 0;  // plain style
        if (strcmp(str, "binary") == 0) {
            binType = 0;  // plain
        } else if (strcmp(str, "binary (multi-Processes)") == 0) {
            binType = 1;  // MPI_style
        } else {
            Abort("Unrecognized tag \"%s\"", str);
        }
        ...
        while (fread(str, sizeof(char), 32, fp) == 32) {
            if (strcmp(str, "dimension") == 0) {
                ...
                for (int iatom = 0; iatom < particle->nAtom; iatom++) {
                    ...
                    fread(&diameter, sizeof(double), 1, fp);
                    fread(&xyz, sizeof(doubleVector), 1, fp);
                    fread(&img, sizeof(intVector), 1, fp);

                    particle->type[tag] = type;
                    particle->diameterScale[tag] = diameter;
                    particle->mass[tag] = particle->massPerType[particle->type[iatom]];
                    vCpy(particle->pos[tag], xyz);
                    vCpy(particle->img[tag], img);
                    particle->id2tag[tag] = tag;
                    particle->tag2id[tag] = tag;
                }
                ...
                for (int iatom = 0; iatom < particle->nAtom; iatom++) {
                    ...
                    fread(&veloc, sizeof(doubleVector), 1, fp);
                    vCpy(particle->veloc[tag], veloc);
                }
            } else {
                int sizeByte = 0;
...
void zeroMomentum(Box* box, Particle* particle, Update* update) {
    if (update->isThermalRun && update->evFlag == 2) {
...
void calcForce(Box* box, Particle* particle, Update* update) {
    ...
    genHalfNebrListPBCimage(box, particle, update);
...
#if false
void calcForceContactNetwork(Box *box, Particle *particle, Update *update, BitArr contactMat) {
    // half-style NebrList
    NebrList *nebrList = &update->nebrList;
    if (particle->isForceValid)
        return;
    ...
    {
        int nPart = particle->nAtom;
        int nbit = nPart * (nPart - 1) / 2;
        BitClearAll(contactMat, nbit);
        
        for (int iatom = 0; iatom < particle->nAtom; iatom++) {
            int itag = particle->id2tag[iatom];
            doubleVector iforce;
            vCpy(iforce, force[iatom]);
            doubleVector iPos;
            vCpy(iPos, particle->pos[iatom]);
            ...
            for (int jth = nebrList->nNebr[iatom].first;
                ...
                double jRc = particle->diameterScale[jatom] * particle->meanDiameter * 0.5;
                double sRc = iRc + jRc;
                int jtag = particle->id2tag[jatom];
                ...
                int ijtag = (itag < jtag ? ((2 * nPart - itag - 1) * itag / 2 + (jtag - itag)) : ((2 * nPart - jtag - 1) * jtag / 2 + (itag - jtag))) - 1;
                BitSet(contactMat, ijtag);
...
int calcExpanCoeff(Box* box, Particle* particle, Update* update, doubleVector* deltaVeloc, double coeff[4]) {  // output unit of coeff[..] is energy.
    ...
    coeff[0] = y1 / sqrt(lenP2) / (update->forceUnits);
    coeff[1] = y2 / lenP2 / (update->forceUnits / pow(update->distanceUnits, 1));
    coeff[2] = y3 / pow(lenP2, 1.5) / (update->forceUnits / pow(update->distanceUnits, 2));
    coeff[3] = y4 / lenP2 / lenP2 / (update->forceUnits / pow(update->distanceUnits, 3));
...
void instant_pureShearXz(Box* box, Particle* particle, Update* update, double deltaStrain) {
    // pure shear strain: \gamma_{ps} = ln(lx_t/lx_ref) = -ln(lz_t/lz_ref);
    // extension in x (contraction in z) is positive strain.
    // pure shear strain is additive.
    adjustImg(box, particle);

    double scaleX = exp(deltaStrain);
...
void backupSimInfo(Box* box, Particle* particle, Box* bakBox, Particle* bakParticle) {
    ...
    bakParticle->__isSortForbidden = particle->__isSortForbidden;
...
void showStructInfo(Box* box, Particle* particle, Update* update) {
    ...
    double sFi = 0;
    for (int iatom = 0; iatom < particle->nAtom; iatom++) {
        sFi += sNorm(force[iatom]);
    }
    sFi /= particle->nAtom;

    printf(" <F_i>: %g; <f_b>: %g (nContact: %d); <f_b>/<F_i>: %g\n", sFi / update->forceUnits, sfij / update->forceUnits, nContact, sfij / sFi);
...
void purgeSim(Box* box, Particle* particle, Update* update) {
    ...
    if (update->toolkit.nToolkit != 0) {
        Info("Warning of memory leaking!");
    }
