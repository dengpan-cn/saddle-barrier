...
cmdArg *findVariable(Variable *var, char *name) {
    char *ptr = name;
    if (strlen(ptr) == 0)
        Abort("Not legal variable name!");
    if (ptr[0] == '-') {
        if (strlen(ptr) < 3 || (ptr[2] == '-'))
            Abort("%s is not legal variable name!", ptr);
        ptr += 2;
    }

    for (int ith = 0; ith < var->nVar; ith++) {
        if (!strcmp(var->cmd[ith].cmdType + 2, ptr))
            return (var->cmd + ith);
    }
    return NULL;
}
void addVariable(Variable *var, char *inputCmd) {
    ...
    if (strlen(tmp) <= 2)
        Abort("Unrecognized cmd %s!", inputCmd);
    if (strncmp(tmp, "--", 2))
        Abort("Unrecognized cmd %s!", inputCmd);
    if (isspace(tmp[2]) || (tmp[2] == '-'))
        Abort("Unrecognized cmd %s!", inputCmd);
    ...
    for (int ith = 1; ith < narg; ith++) {
        if (!strncmp(arg[ith], "--", 2))
            Abort("Add ONE variable!");
...
int findToolkit(Toolkit *toolkit, char *name) {
    for (int ith = 0; ith < toolkit->nToolkit; ith++) {
        if (!strcmp(name, toolkit->toolkitName[ith]))
            return ith;
    }
    return -1;
}
...
int setSignalHandler(void) {
    if (screenOutputReadConf)
        safeFprintf(stderr, "SetSignalHandler ...\n");
